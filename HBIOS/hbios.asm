...
#IF (PLATFORM == PLT_MT)
; CBIOS also uses HCB_RAMBANKS and HCB_ROMBANKS but has hardcoded offset of 4 banks
; Offset Included here to avoid changes to CBIOS
CB_RAMBANKS	.DB	BID_RAMDN - BID_RAMD0 + 1 + 4
CB_ROMBANKS	.DB	BID_ROMDN - BID_ROMD0 + 1 + 4
#ELSE
...
;
;	End of HBIOS copy to RAM not used for PLT_MT
#ENDIF
;
...
;
#IF (PLATFORM == PLT_MT)
	XOR	A			; clear device tables in ram
	LD	(CIO_CNT),A		; copy from rom to ram page normally clears these
	LD	(DIO_CNT),A
	LD	(VDA_CNT),A
...
;
#IF ((PLATFORM != PLT_N8) & (PLATFORM != PLT_MK4) & (PLATFORM != PLT_RC) & (PLATFORM != PLT_RC180) & (PLATFORM != PLT_MT))
...
HB_WAITSEC1:
#IF ((PLATFORM == PLT_SBC) | (PLATFORM == PLT_ZETA) | (PLATFORM == PLT_ZETA2)| (PLATFORM == PLT_RC) | (PLATFORM == PLT_MT))
